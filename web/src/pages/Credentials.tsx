...
function CreateForm({ onDone }: { onDone: () => void }) {
  ...
  const m = useMutation({
    mutationFn: () => api.post<Credential>('/credentials', { name, kind, secret }),
    onSuccess: onDone,
  })
  return (
    <div className="card">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 10 }}>
        ...
        <label>{kind.startsWith('snmp') ? 'Community / secret' : 'Password / secret'}
          <input style={input} type="password" value={secret} onChange={(e) => setSecret(e.target.value)} autoComplete="new-password" />
        </label>
      </div>
      <div style={{ marginTop: 12 }}>
        <button style={btn} disabled={!name || !secret || m.isPending} onClick={() => m.mutate()}>
