...
import { Search as SearchIcon, Boxes, Building2, ClipboardList, Wrench, Network } from 'lucide-react'
...
export function SearchPage() {
  ...
  const devHits = (devices.data ?? []).filter((d) =>
    t && (d.name.toLowerCase().includes(t) || (d.primary_ip ?? '').includes(t) || (d.vendor ?? '').toLowerCase().includes(t)
      || (d.serial ?? '').toLowerCase().includes(t) || (d.hostname ?? '').toLowerCase().includes(t) || d.category.includes(t))).slice(0, 25)
  ...
  const run = () => { setQ(term); setParams(term.trim() ? { q: term.trim() } : {}) }
      ...
      <div className="card">
        <div className="search-box" style={{ marginBottom: 0 }}>
          <input autoFocus value={term} onChange={(e) => setTerm(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && run()}
            placeholder="Search anything — 172.21.15.44 · aa:bb:cc:dd:ee:ff · SW-CORE-01 · Aruba · Hotel A" />
          <button onClick={run}>Search</button>
        </div>
      </div>

      {!t && <EmptyState icon={SearchIcon} title="Search the whole platform" message="Find devices by name/IP/serial/vendor, locations, work orders, systems, and resolve a MAC/IP to its switch port." />}
