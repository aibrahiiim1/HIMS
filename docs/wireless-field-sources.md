...
| SNMP fallback pack selection | both | When an enabled vendor profile is removed, collection falls back to SNMP/SSH. A controller added ONLY via the REST/XML "Add controller" flow has no SNMP fingerprint, so the wireless SNMP path probes sysObjectID/sysDescr (and persists them) before matching the MIB pack — without this it would match the broad category and pick the wrong vendor pack. **Limitation:** SNMP fallback still requires an SNMP v2c credential bound to the device (the REST/XML add flow does not SNMP-probe at add time); once one is bound, the probe selects the vendor-exact pack and rows populate (verified live: Ruckus ZD → 233 APs via `snmp_wireless_mib`). |
