...
const countInventoryOnlyDevices = `-- name: CountInventoryOnlyDevices :one
...
func (q *Queries) CountInventoryOnlyDevices(ctx context.Context) (int64, error) {
	row := q.db.QueryRow(ctx, countInventoryOnlyDevices)
	var column_1 int64
	err := row.Scan(&column_1)
	return column_1, err
}
...
const listBMCWithoutRedfishInventory = `-- name: ListBMCWithoutRedfishInventory :many
...
type ListBMCWithoutRedfishInventoryRow struct {
	ID               uuid.UUID   `json:"id"`
	Name             string      `json:"name"`
	PrimaryIp        *netip.Addr `json:"primary_ip"`
	Vendor           *string     `json:"vendor"`
	Subtype          string      `json:"subtype"`
	RedfishReachable bool        `json:"redfish_reachable"`
...
func (q *Queries) ListBMCWithoutRedfishInventory(ctx context.Context) ([]ListBMCWithoutRedfishInventoryRow, error) {
	rows, err := q.db.Query(ctx, listBMCWithoutRedfishInventory)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListBMCWithoutRedfishInventoryRow{}
	for rows.Next() {
		var i ListBMCWithoutRedfishInventoryRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.PrimaryIp,
...
const markDeviceInventoryOnly = `-- name: MarkDeviceInventoryOnly :one
UPDATE devices SET is_inventory_only = $2, updated_at = now()
WHERE id = $1 AND deleted_at IS NULL
RETURNING id, location_id, primary_ip, hostname, name, vendor, model, serial, os_version, category, status, driver, credential_id, last_discovery_at, last_monitoring_at, metadata, created_at, updated_at, deleted_at, vlan, device_class, location, os_family, confidence_score, classification_evidence, classification_locked, subtype, notes, criticality, monitoring_enabled, manual_classification_reason, is_virtual, cctv_credential_id, web_scheme_pref, web_port_pref, web_alt_ports, web_notes, web_last_ok, web_last_ok_at, web_last_proto, web_last_scheme, web_last_port, web_last_credential_id, web_pref_proto, is_inventory_only, reachability_signal, reachability_confidence
`

type MarkDeviceInventoryOnlyParams struct {
	ID              uuid.UUID `json:"id"`
...
func (q *Queries) MarkDeviceInventoryOnly(ctx context.Context, arg MarkDeviceInventoryOnlyParams) (Device, error) {
	row := q.db.QueryRow(ctx, markDeviceInventoryOnly, arg.ID, arg.IsInventoryOnly)
	var i Device
	err := row.Scan(
		&i.ID,
		&i.LocationID,
...
const setDeviceCCTVCredential = `-- name: SetDeviceCCTVCredential :exec
UPDATE devices SET cctv_credential_id = $2, updated_at = now() WHERE id = $1
`

type SetDeviceCCTVCredentialParams struct {
	ID               uuid.UUID  `json:"id"`
...
func (q *Queries) SetDeviceCCTVCredential(ctx context.Context, arg SetDeviceCCTVCredentialParams) error {
	_, err := q.db.Exec(ctx, setDeviceCCTVCredential, arg.ID, arg.CctvCredentialID)
	return err
}
...
const setDeviceWebSuccess = `-- name: SetDeviceWebSuccess :exec
UPDATE devices SET
...
type SetDeviceWebSuccessParams struct {
	ID                  uuid.UUID  `json:"id"`
...
func (q *Queries) SetDeviceWebSuccess(ctx context.Context, arg SetDeviceWebSuccessParams) error {
	_, err := q.db.Exec(ctx, setDeviceWebSuccess,
		arg.ID,
		arg.WebLastProto,
		arg.WebLastScheme,
		arg.WebLastPort,
		arg.WebLastOk,
		arg.WebLastCredentialID,
	)
	return err
}
