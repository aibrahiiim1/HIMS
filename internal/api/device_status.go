...
func (m *statusMaps) statusDataQualityIssues(devs []db.Device, now time.Time) []dqIssue {
	...
	add("credential_not_authorized", "Credential not authorized on host", "The host denied access after a credential reached it. Over WinRM/PSRP this is a proven authorization denial (UAC LocalAccountTokenFilterPolicy, remote-logon rights, group membership, WinRM/DCOM policy) — fix host policy or use an authorized credential. When the ONLY signal is WMI/DCOM \"Access is denied\" (0x80070005) and WinRM is disabled, it is AMBIGUOUS: it can equally mean a WRONG local password for this host, because DCOM returns the same code for both. Enable WinRM on the host to disambiguate, or verify this host's local admin password.", "warning", notAuthorized)
