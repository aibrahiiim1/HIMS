...
func (s *Server) routes() {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Post("/discovery/wmi-collector-test", s.wmiCollectorTest)
		...
		r.Get("/inventory/asset-summary", s.assetSummaryHandler)  // canonical asset vs endpoint counting model
		r.Get("/inventory/assets", s.assetsHandler)               // one row per REAL asset (SVI/BMC folded)
		...
		r.Post("/vendor-profiles", s.createVendorProfile)
		r.Patch("/vendor-profiles/{id}", s.updateVendorProfile)
		r.Delete("/vendor-profiles/{id}", s.deleteVendorProfile)
		r.Post("/vendor-profiles/{id}/test", s.testVendorProfile)
		r.Post("/vendor-profiles/{id}/run-collection", s.runVendorProfileCollection)
