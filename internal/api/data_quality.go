...
func (s *Server) dataQuality(w http.ResponseWriter, r *http.Request) {
	...
	if am, aerr := s.deviceAccessMap(ctx); aerr == nil {
		...
		addIssue("windows_wmi_access_denied", "Windows WMI access denied", "WMI/DCOM returned \"Access is denied\" (0x80070005). This is AMBIGUOUS over DCOM: it can mean the account is not authorized (DCOM launch/activation or namespace permissions, UAC remote restrictions) OR a wrong local password for this host — DCOM returns the same code for both and never surfaces a distinct logon failure. Enable WinRM on the host to disambiguate (and get a full-token collection), or verify this host's local admin password.", "warning", wmiAccessDenied)
