...
func (s *Server) runOSCollection(ctx context.Context, d db.Device) osCollectResult {
	...
	for _, cd := range cands {
		...
		// Device-row identity enrichment (serial/UUID/vendor/model/os/hostname) is now done
		// inside osinv.Persist so EVERY path (direct WinRM, native collector, WMI, relay agent)
		// enriches the device consistently — previously only this direct path did, so agent/
		// native-collected servers never got their chassis serial onto the device row.
...
func credKindMatchesMethod(kind, method string) bool {
	switch method {
	case "winrm", "wmi":
		// Windows: a unified "windows" credential (and the legacy winrm/wmi kinds) are all
		// user/pass and work over either transport, so the direct WinRM path and the agent WMI
		// path try the SAME candidate set — they converge to the same managed state when a valid
		// credential exists.
